...
class EvaluationHelper:
    ...
    def calculate_metrics(self, generate_files_path, groundtruth_path, same_name, limit_num=None, calculate_psnr_ssim=True, calculate_lsd=False, recalculate=False):
        ...
        fad_score = self.frechet.score(generate_files_path, groundtruth_path, limit_num=limit_num, recalculate=False)
        ...
        if("embed" in featuresdict_1.keys() and "embed" in featuresdict_2.keys()):
            metric_fid = calculate_fid(
                featuresdict_1, featuresdict_2, feat_layer_name="embed"
            )
        ...
        for waveform, filename in tqdm(dataloader):
            try:
                ...
                with torch.no_grad():
                    ...
                    # featuresdict["logits"] = self.passt_model_logits(waveform)
                    # featuresdict["embed"] = self.passt_model_embed(waveform)
                    # featuresdict["2048"] = self.passt_model(waveform)[1]
                    # breakpoint()
